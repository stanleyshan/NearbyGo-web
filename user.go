...
func checkUser(username, password string) bool {
	...
	termQuery := elastic.NewTermQuery("username", username) //NewTermQuery  new 出一個 query
...
func addUser(username, password string) bool {
	...
	if queryResult.TotalHits() > 0 { //反回有幾個match到的結果
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if checkUser(u.Username, u.Password) {
		token := jwt.New(jwt.SigningMethodHS256) //new token
		claims := token.Claims.(jwt.MapClaims)  //Claims是key value pair，強制轉換成map。方便查找，因為原本是json格式，不方便查找
		/* Set token claims */
		...
		w.Write([]byte(tokenString))   //寫進response
	} else {
